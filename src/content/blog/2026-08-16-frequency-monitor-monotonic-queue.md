...
很多人卡在「拿到一道题不知道要不要用单调栈」。这块不是靠灵感，而是**一条可复制的思维链**：

**Step 1 — 先找一个一定对的暴力枚举思路。** 别急着套数据结构，先把"对每个东西，怎么靠穷举算出答案"想清楚。单调栈的前提是把问题**翻译成了一种可枚举的形式**。

**Step 2 — 在暴力里找出重复且开销大的子操作。** 暴力慢，往往因为对每个元素都要"回头看"——回扫找某个边界、某个前驱/后继。这一步发现了瓶颈，后面才有优化对象。

**Step 3 — 把这个子操作识别成 next smaller / next greater 模式。** 一旦你发现需求是「求每个元素**左边/右边第一个比它大（或小）**的元素/位置」，单调栈的标志就点亮了。这是整个家族共同的内核：

| 需求 | 数据结构 | 单调性 |
|------|---------|--------|
| 每个元素右边第一个更小 | 单调递增栈 | 弹栈时结算 |
| 每个元素右边第一个更大 | 单调递减栈 | 弹栈时结算 |
| 滑动窗口内的最值 | 单调队列 | 队首恒为最值 |

**Step 4 — 为什么它能 O(n)？** 每个元素**只入栈一次、只出栈一次**，把"每次都要从当前位置往回扫 O(n)"的重复劳动，分摊成均摊 O(1)。这是"用有序性换时间"的典型。

**触发判据 checklist**（命中即点亮"单调栈"灯）：
- ✅ 对序列/数组中的**每个元素**，需要知道**左边/右边第一个比它大或小**的元素/位置
- ✅ 元素之间是**一维、有序**关系（数组、柱状图、温度、每日价格…）
- ✅ 朴素做法要**反复往回扫描** → 有 O(n²) 的味道
- ✅ 需要维护"当前未结算的候选集合"，且**后进来先结算**（LIFO ↔ 栈）

**一句话：想到单调栈 = 把问题翻译成"求每个元素左右第一个更小/更大"这个模式 → 认出这是 next smaller/greater 问题 → 条件反射到单调栈。** 前半（翻译+识别）靠推理，最后一步靠练过的经验。
