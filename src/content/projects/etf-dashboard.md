...
- **标的**：中证红利、红利低波、港股央企红利、中证电池主题、港股通红利低波（可编辑 `etf-dashboard/watchlist.json` 增删品种）
- **数据源**：中证官网（5年 PE 分位）+ 天天基金（估值/ROE）+ 腾讯行情/K线；底层取数统一走「统一数据源层」（含缓存与限速，见 Daily Skills 项目页）
